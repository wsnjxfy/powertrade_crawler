...
def load_live_cases() -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, list) or len(payload) != 12:
        raise ValueError("Live evaluation fixture must contain exactly 12 cases.")
    return payload
...
def run_live_evaluation(
    ...
    selected_cases = live_cases[:limit] if limit is not None else live_cases
    ...
    for index, case in enumerate(selected_cases, start=1):
        ...
        result = loop.chat(prompt)
        proposed_tools = [
            event["detail"].get("tool_name")
            for event in result.events
            if event["stage"] == "tool_proposed"
        ]
        action_case = bool(case["approval_expected"])
        expected_tool = str(case["expected_tool"])
        tool_correct = expected_tool in proposed_tools
        parameter_correct = not any(
            event["stage"] == "tool_validation_failed" for event in result.events
        )
        approval_correct = (
            result.status.value == "awaiting_approval"
            if action_case
            else result.status.value != "awaiting_approval"
        )
        passed = bool(
            tool_correct
    ...
    approval_rate = (
        sum(row["status"] == "awaiting_approval" for row in action_rows)
        / len(action_rows)
        if action_rows
        else 1.0
    )
...
def run_evaluation(
    *,
    ...
    if mode == "offline":
        ...
        report = run_live_evaluation(limit, progress_callback)
    else:
